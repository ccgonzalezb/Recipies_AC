...
const db = mysql.createConnection({
    host: 'localhost',
    user: 'root',
    password: 'Camilo1994.', // <--- ¡PON TU CLAVE AQUÍ!
    database: 'recetario_hexagonal'
});
...
router.get('/:categoria', (req, res) => {
    ...
    const sql = "SELECT * FROM recetas WHERE categoria = ? ORDER BY RAND() LIMIT 1";
...
router.post('/', (req,res) =>{
    ...
    const imagenFinal = imagen || 'https://via.placeholder.com/500?text=Sin+Imagen';

    // 4. La consulta SQL
    const sql = 'INSERT INTO recetas (titulo, categoria, ingredientes, preparacion, imagen) VALUES (?,?,?,?,?)';

    db.query(sql,[titulo, categoria, ingredientesJSON, preparacion, imagenFinal], (err, result) =>{
        if (err){
